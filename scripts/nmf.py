...
def symnmf_admm(A, k, H=None, maxiter=1e3, tol=1e-5, sigma=1):
    ...
    Returns W and a boolean index to specify which samples were included in
        calculation of W
    """
    A = A.copy()
    A[A < 0] = 0

    # drop rows and columns which are not measured in this layer
    rindex = ~np.isnan(A).all(axis=1)
    cindex = ~np.isnan(A).all(axis=0)
    A = A[rindex][:, cindex]
